<template>
  ...
  <!--                  <i class="fa fa-spinner fa-spin" v-if="loading_analysis"></i>-->
          ...
          <!-- AIRLINE NAME -->
          <div class="col-12 col-md-4">
              <h5>{{ returndata.airline_name }}</h5>
              <h5 class="mt-2">{{ returndata.flight_number }}</h5>
            </div>
            <!-- GO FLIGHT -->
            <div class="col-6 col-md-2 mt-3 mt-md-0">
              <h5 class="mt-2">{{ returndata.go_time }}</h5>
              <h5 class="mt-2">{{ returndata.go_date }}</h5>
            </div>
            <!-- RETURN FLIGHT -->
            <div class="col-6 col-md-2 mt-3 mt-md-0">
              <h5 class="mt-2">{{ returndata.return_time }}</h5>
              <h5 class="mt-2">{{ returndata?.return_date }}</h5>
            </div>
            <!-- PRICE -->
            <div class="col-12 col-md-4 mt-3 mt-md-00">
              <div class="d-flex algin-items-center justify-content-between">
                <h5>قیمت هر نفر:</h5>
                <h5 class="text-center">{{ perPerson(returndata.price) }} تومان</h5>
              </div>
              <hr>
              <div class="d-flex algin-items-center justify-content-between">
                <h5>قیمت کل:</h5>
                <h5 class="text-center">{{ totalPerson(returndata.price) }} تومان</h5>
              </div>
            </div> 
          ...
          <div class="col-12 mt-3 ">
              <div class="row align-items-center">
                <div class="col-12 col-md-3">
                  <h5 class="">{{ returndata.providers[0].provider_name }}</h5>
                </div>
                <div class="col-12 col-md-3 mt-3 mt-md-0">
                  <button class="btn btn-info w-100" v-on:click="show = !show">
                    <span>
    <!--                  <i class="fa fa-spinner fa-spin" v-if="loading_analysis"></i>-->
                ...
                <div class="col-12 col-md-3 mt-3 mt-md-0">
                  <button class="btn btn-primary w-100" v-on:click="show = !show">
                    <span v-if="show">بستن</span>
                    <span v-else>سایر ({{ returndata.providers.length }})</span>
                  </button>
                </div>
                <div class="col-12 col-md-3 mt-3 mt-md-0">
                  <a target="_blank" :href="returndata.providers[0].buy_link" class="btn btn-success w-100">نمایش در سایت</a>
                </div>
              </div>
            </div>
  ...
  export default {
    ...
    props: {
      data: Object,
      returndata:Object,
    //   flight_return:Object,
      adults: String
    },
    // mounted(){
    //     console.log("return flight data:" , this.returndata);
    // }
  }
  </script>
